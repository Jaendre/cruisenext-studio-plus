#!/usr/bin/env python3
"""CruiseNext Itinerary Studio Plus local server."""
...
def fetch_bytes(url, data=None, headers=None, timeout=30):
    ...
    request = Request(url, data=data, headers=req_headers, method="POST" if data is not None else "GET")
    try:
...
def is_allowed_ncl_url(value):
    ...
    return host in {"www.ncl.com", "ncl.com"} and "/cruises/" in path
...
def search_ncl_pages(ship_name, month, year):
    ...
    for search_url in search_urls:
        try:
            ...
            print("search failed", search_url, error)
            continue
...
def extract_ship_name(html, fallback):
    patterns = [
        r"on Norwegian ([A-Za-z]+)",
        r"aboard Norwegian ([A-Za-z]+)",
        r'"name"\s*:\s*"Norwegian ([A-Za-z]+)"',
        r"Norwegian (Aqua|Aura|Bliss|Breakaway|Dawn|Encore|Epic|Escape|Gem|Getaway|Jade|Jewel|Joy|Luna|Pearl|Prima|Sky|Spirit|Star|Sun|Viva)",
        r"Pride of America",
    ]
...
def add_days(iso_date, amount):
    ...
    if not date:
        return iso_date
    return (date + timedelta(days=amount)).strftime("%Y-%m-%d")


def extract_events(html):
    pattern = re.compile(
        r'"date":(\d{12,13}),"portCode":"([A-Z0-9]+)","portName":"([^"]+)","portTag":"([^"]*)","event":"([AD])"'
    )
    events = []
    seen = set()
    for match in pattern.finditer(html):
        item = {
            ...
            "portCode": match.group(2),
            "portName": decode_entities(match.group(3)),
            "event": match.group(5),
        }
...
def extract_cruise_length(html):
    match = re.search(r"analyticsProductLength\s*=\s*\[[^\]]*?(\d+)\s*Days", html, flags=re.I)
    if match:
        return int(match.group(1))
    match = re.search(r"(\d+)\s*-\s*day cruise", html, flags=re.I)
    if match:
        return int(match.group(1))
    match = re.search(r"(\d+)\s+Days\.\s*\d+\s+Ports", html, flags=re.I)
    if match:
        return int(match.group(1))
    return None
...
def infer_length_from_events(events):
    ...
    for event in events[1:]:
        ...
        date = parse_iso(iso)
        if not date or not first_date:
            continue
        delta = (date - first_date).days
        if delta >= 3 and event["portName"] == first_port:
            return delta
        if delta >= 4 and event["event"] == "A" and delta <= 16:
            # likely the first disembark if names differ; keep scanning
            pass
    unique_days = []
    seen = set()
    for event in events:
        ...
        if iso in seen:
            continue
        seen.add(iso)
        unique_days.append(iso)
        if len(unique_days) > 16:
            break
    if len(unique_days) >= 2:
        start = parse_iso(unique_days[0])
        end = parse_iso(unique_days[min(7, len(unique_days) - 1)])
        if start and end:
            return max((end - start).days, 3)
    return 7


def extract_sail_dates(html, month, year):
    dates = set()
    for match in re.finditer(r"sailDate=(\d{12,13})", html):
        dates.add(iso_from_timestamp(int(match.group(1))))
    for match in re.finditer(r'"sailDate":(\d{12,13})', html):
        dates.add(iso_from_timestamp(int(match.group(1))))
...
def build_template_days(events):
    if not events:
        return []

    first = datetime.fromtimestamp(events[0]["date"] / 1000, tz=timezone.utc).date()
    last = datetime.fromtimestamp(events[-1]["date"] / 1000, tz=timezone.utc).date()
    by_day = {}
    for event in events:
        key = iso_from_timestamp(event["date"])
        by_day.setdefault(key, event["portName"])

    days = []
    cursor = first
    while cursor <= last:
        iso = cursor.strftime("%Y-%m-%d")
        days.append({"place": by_day.get(iso, "Sea Day")})
        cursor += timedelta(days=1)

    if days and days[-1]["place"] == "Sea Day":
        days[-1]["place"] = by_day.get(last.strftime("%Y-%m-%d"), days[-1]["place"])
    return days
...
def ships_match(left, right):
    a = normalize_ship(left)
    b = normalize_ship(right)
    if not a or not b:
        return True
...
def parse_ncl_itinerary_page(page_url, ship_name, month, year):
    ...
    length = extract_cruise_length(html) or infer_length_from_events(events)
    length = max(int(length), 2)

    ports_by_day = {}
    for event in events:
        key = iso_from_timestamp(event["date"])
        ports_by_day.setdefault(key, event["portName"])
    ...
    if not sail_dates:
        ...
        if first_date:
            for event in events[1:]:
                ...
                if not date:
                    continue
                if event["portName"] == first_port and (date - first_date).days >= length:
                    if iso not in sail_dates:
    ...
    for start_date in sail_dates:
        ...
        for index in range(length + 1):
            ...
            days.append({
                "day": index + 1,
                "date": iso,
                "place": ports_by_day.get(iso, "Sea Day"),
            })
        ...
        if days[0]["place"] == "Sea Day":
            days[0]["place"] = real_ports[0]

        itineraries.append({
            "ship_name": resolved_ship or ship_name or "Norwegian",
            "start_date": start_date,
            "end_date": days[-1]["date"],
            "cruise_length": length,
            "embark": days[0]["place"],
            "disembark": days[-1]["place"],
            "days": days,
            "source_url": page_url,
        })
...
def dedupe_itineraries(items):
    ...
    for item in items:
        key = (
            normalize_ship(item.get("ship_name", "")),
            item.get("start_date"),
            item.get("end_date"),
            "|".join(day.get("place", "") for day in item.get("days", [])),
        )
        if key in seen:
            continue
        seen.add(key)
        ...
        for part in parsed.iter_parts():
            ...
            if filename_match:
                files.setdefault(name, []).append(
                    (filename_match.group(1), part.get_content_type(), payload)
                )
            else:
...
def proxy_extract(files):
    ...
    status, content_type, response_body = fetch_bytes(
        ORIGINAL_EXTRACT,
        data=body,
        headers={
    ...
    if "application/json" not in (content_type or ""):
        return 502, {
            "error": "Screenshot analysis backend returned an unexpected response. The original extract service may be waking up — try again in a few seconds."
        }
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/extract":
            try:
                files_map, _fields = parse_multipart(self)
                files = files_map.get("images") or []
                if not files:
                    self._json(400, {"error": "Please upload at least 1 screenshot."})
                    return
                ...
                self._json(502, {
                    "error": "Could not reach the screenshot analysis backend. NCL lookup still works without screenshots."
                })
            return
        ...
        if self.path == "/api/ncl-lookup":
            ...
            if not ship_name and not direct_url:
                self._json(400, {"error": "Enter a ship name or paste an NCL itinerary URL."})
                return
            ...
            if direct_url:
                if not is_allowed_ncl_url(direct_url):
                    self._json(400, {"error": "That URL is not an ncl.com itinerary page."})
                    return
            ...
            if not page_urls:
                self._json(404, {"error": "No public NCL.com itinerary pages were found for that ship and date."})
                return
            ...
            for url in page_urls[:8]:
                try:
                    itineraries.extend(parse_ncl_itinerary_page(url, ship_name, month, year))
                except Exception as error:
            ...
            if not unique:
                self._json(404, {
                    "error": "Found NCL pages, but could not convert them into itinerary templates.",
                    "details": errors[:4],
                })
                return
